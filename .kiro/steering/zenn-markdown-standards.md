...
#### 見出し4
```
- 説明的で階層的なヘッダー（H1-H6）を使用しなければならない (MUST)
- 記事全体で一貫したヘッダー構造を維持しなければならない (MUST)
- ヘッダーレベルの飛び越し（H1 → H3）を避けなければならない (MUST NOT)
...
- 順序付きリストは同じ番号 (1., 1., 1.) を使用してもよい (MAY)
...
`インラインコード`
```
- 強調には**太字**、軽い強調には*斜体*を使用しなければならない (MUST)
- 技術用語やコマンドには`インラインコード`を使用する必要がある (SHOULD)

### リンク
```markdown
[アンカーテキスト](リンクのURL)
```
- 説明的なリンクテキストを使用、「ここ」や「これをクリック」を避けなければならない (MUST)
- 内部コンテンツには相対リンクを優先しなければならない (MUST)
...
- すべての画像にalt textを含めなければならない (MUST)
- 画像にはZennの画像アップロードサービスまたはGitHubリポジトリを使用しなければならない (MUST)
...
- シンタックスハイライト用に言語を指定する必要がある (SHOULD)
- ファイル内容を表示する際は説明的なファイル名を使用する必要がある (SHOULD)
- 複雑なコード例には日本語でコメントを含める必要がある (SHOULD)
- 断片的なものより完全で実行可能な例を優先する必要がある (SHOULD)
