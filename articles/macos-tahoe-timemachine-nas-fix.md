...
日本語環境のmacOSは、新規バックアップ中に `<ホスト名>のバックアップ` という名前でディスクイメージを作成します。しかし、macOS Tahoe では、濁点または半濁点のある文字（`バックアップ`の`バ`と`プ`など）がディスクイメージのファイル名に含まれていると正しく動作しません。

## 解決策

1. `システム設定 > 一般 > 言語と地域 > 優先する言語` で、先頭を`English`に
1. `システム設定 > 一般 > 情報 > コンピュータ名` に日本語が含まれる場合はアルファベットのみに変更
1. macOSを再起動
1. 新規 Time Machine バックアップを取得
1. `System Settings > General > Language & Region` で、先頭を`日本語`に戻す
1. `System Settings > General > About > Name` を元に戻す

![macOS システム設定の言語と地域で優先言語をEnglishに変更した画面](/images/macos-tahoe-timemachine-nas-fix/language-settings-english.png)

英語環境では `Backups of <Hostname>` というディスクイメージ名で作成されるため、問題が起きません。バックアップを一度取得した後は既存の `Backups of <Hostname>` という名のディスクイメージを使うので、優先言語やコンピュータ名を日本語に戻しても問題ありません。
